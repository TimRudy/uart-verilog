...
[`CodeCoverageIndex.md`](CodeCoverageIndexEx.md)

- Lists all the non-trivial `if-else` branches in the code. Each one references a test that covers it.

- The code coverage can, however, be seen in a reverse, more informative, index in the tests below: From each test, click to see the highlighted code lines relevant to the behaviour. This helps to relate the behaviour description to the HDL `if` statements, the signal values, the signal changes, the FSM state - to help you pick out the "flow" leading up to a moment seen visually on the trace.
...
Test results would be identical if a loopback configuration was used; that is, testing on only one chip, connecting the tx pin to the rx pin. The equivalence is demonstrated by test variant [\#1a](1a.v): It's the same as test #1 but with the one-chip configuration. The delta of the test bench setup can be seen here: [1.v &larr;&rarr; 1a.v](https://github.com/TimRudy/uart-verilog/compare/main..TimRudy:diff-test-1a).

Group 1 shows the communication as an integrated whole:
...
- `txStart` must be asserted over the time period when `45` data is accepted for transmit

- `txEn` and `rxEn` must hold for the transmit and receive durations otherwise the transmission halts in the middle

- `rxDone` is the output that can be monitored for the purpose of pulling out the `out` data `45`, because `out` is only available for a limited time
...
<details>
<summary>Mismatch in transmit, receive clocks: Tolerance</summary>
...
- These two tests tweak the parameter `RX_OVERSAMPLE_RATE` to distort the relation between `txClk` and `rxClk`

- By design, the frequency ratio is `1:16`, but in reality the transmitting and receiving UARTs' clocks are independent, so unsynchronized. A degree of synchronization occurs through the UART protocol itself, though: Every 8 bits the receiver waits and listens for the idle-to-start transition. See the idle waiting interval in other tests, [e.g. #4, #5](#4-two-successful-frames-second-is-shortened-due-to-external-reset)

- The idle interval between each 8-bit packet gives a "reset" for sampling drift (from the precise middle of each bit) that may occur on the receiver side by the time of the eighth bit and the stop bit being sampled
...
- `RX_OVERSAMPLE_RATE` is outside the range where the sampling of 8 bits by the receiver actually aligns with the 8 bits, so this demonstrates how communication will go wrong when two systems don't have the same UART protocol configured, or the same nominal clock rate
...
<details>
<summary>Two transmission frames: Enabling, disabling and using txStart signal</summary>
...
- `IDLE` state is ended by the `txStart` signal being clocked in

- The receiver is very much the same, except it relies on the transmitter to wake it from `IDLE` state `001`

- Note transmitter `out`, receiver `in`/`in_sample`: The `1` value of these is "mark" state and signals no transmit in progress; the drop to `0` signals to start receiving
...
- `txStart` does not go low for this mode; so for each frame, the `in` data just needs to be set up in time to be captured in `in_data` and transmitted
...
- This shows a third transmit starting, because `txStart` goes low too late at the end of second transmit

- This test bench uses a feedback method of control to shut off `txStart`; so it's suggestive of ideas for external control of this UART; but these tests do not go into how you can use outputs for external control, and how to decide the timing of inputs

- The test benches rely otherwise, however, on tuned timings to present the inputs according to the intent of the test - in other words, empirical or ad hoc timings. Examples to illustrate:

  - Interval `#11500` is used in test [#13](#13-turbo_frames--0): Because each cycle is about 11.5ms, this gives prep or set-up times mid-way through each transmit for pushing each next byte (the `in` transitions relative to the `done` pulses)
...
- Documentation for the `8-N-1`, `8-N-2` modes: `Uart8Transmitter` [`header`](https://github.com/TimRudy/uart-verilog/blob/a805332/Uart8Transmitter.v#L22-L38)

- This mode `8-N-1` provides the maximum bandwidth: Effective data rate of 80%, over the line, because 10 bits transmitted for each 8-bit packet

- But I gave the Verilog code a default of `8-N-2`, `TURBO_FRAMES = 0`, because it fits the project's purpose, of: Simulation & testing, either for the UART's own sake or to support other projects in development; and: Education, visualization. So the UART might as well be more bullet-proof in use, and if you are getting specific about your use case, then you'll set the parameters
...
- The Verilog that implements the `TURBO_FRAMES` feature (see `Uart8Transmitter` refs above), deserves a note for the reader
    ...
    The [code](https://github.com/TimRudy/uart-verilog/blob/a805332/Uart8Transmitter.v#L112-L130) for `STOP_BIT` state waits in that state for either 1 tick or 2 - but how, and why, is it using that `done` variable?
    ...
    Those are hints to reveal how the `if-else` code works to introduce a single-clock-tick delay. The logic could present itself more clearly, if there was a separate new variable, or another state, but for convenience and economy it uses variable `done` that is boolean and already at hand.
...
- Byte `99` misses its deadline; however, it is still present at the input for next data capture, so it is transmitted
...
Group 2 tests focus on the receiver RX part of the UART.

The TX module is fairly deterministic, and it's been tested by all the transmits of the Group 1 scenarios.

The RX module has a tougher job, because it receives input in an arbitrary signal pattern and must make sense out of it; it must lock on and accept good serial data (forming a frame), or must reject an ill-formed data stream. So, these test signals don't have to come from a well-behaved or realistic TX module. You could consider them from a potentially "malicious" transmitter.

If an input waveform meets the protocol requirements, bits will be read correctly and the output will be the 8-bit byte expected; if not, the output can be an error signal or can be garbled data.

These are fine-grained tests to nail down the behaviour of this device by exploring the range of signal waveforms possible (mainly the variety of timings; and the signal held low when it should revert to high or vice versa; in addition, high-low or low-high glitches). Variants of tests are necessary to do this, as I'll mention below.

The intent is that there is full coverage so no edge cases are missed. The device's behaviour is supposed to be well-defined and fully determined/understood for all input conditions.
...
`Uart8Receiver:` [`141`](https://github.com/TimRudy/uart-verilog/blob/a805332/Uart8Receiver.v#L141), [`158`](https://github.com/TimRudy/uart-verilog/blob/a805332/Uart8Receiver.v#L158)
...
- Start is recognized only at a later bit, after high signal time is first met
...
<details>
<summary>Transition between two frames: Overlapping done and error signals</summary>
